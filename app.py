...
async def main(message: cl.Message):
    try:
        ...
        context = "\n".join([d.page_content for d in docs]) if docs else "لا يوجد سياق محدد."

        # بناء الـ Prompt
        prompt = f"""أجب على سؤال المستخدم بناءً على السياق المرفق فقط.
السياق:
{context}

السؤال: {message.content}"""

        # إرسال الطلب لـ OpenRouter
        response = await llm_client.chat.completions.create(
            model="ox/ox-alpha", # أو استبدله بأي نموذج مجاني مثل google/gemini-flash-1.5
